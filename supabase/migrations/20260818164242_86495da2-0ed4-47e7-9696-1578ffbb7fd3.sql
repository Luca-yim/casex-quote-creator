CREATE TYPE public.app_role AS ENUM ('external','sales_rep','estimator','admin');
...
CREATE POLICY "Users can view own profile" ON public.profiles FOR SELECT TO authenticated USING (auth.uid() = id OR public.has_role(auth.uid(),'estimator') OR public.has_role(auth.uid(),'admin'));
