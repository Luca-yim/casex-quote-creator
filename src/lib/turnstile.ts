...
/**
 * True when a site key is configured and the widget must be rendered.
 *
 * There is no bypass: Supabase Auth enforces CAPTCHA server-side, so every
 * sign-in, sign-up and anonymous session needs a real Turnstile token.
 */
