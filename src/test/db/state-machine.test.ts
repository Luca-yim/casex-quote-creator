...
describe("quote state machine (database guard)", () => {
  it("allows draft → submitted_for_review by the owning rep", async (ctx) => {
  ...
  it("rejects draft → approved (skipping review)", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
  ...
  it("rejects a rep approving their own submitted quote", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
  ...
  it("allows an estimator to move submitted_for_review → under_review", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
    await transitionQuote(actors.rep!, id, "submitted_for_review");
  ...
  it("allows an estimator to return estimator_adjusted → draft", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
    await transitionQuote(actors.rep!, id, "submitted_for_review");
  ...
  it("allows approve then send_to_customer by the owning rep", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
    await transitionQuote(actors.rep!, id, "submitted_for_review");
  ...
  it("rejects reopening an accepted quote", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
    await transitionQuote(actors.rep!, id, "submitted_for_review");
  ...
  it("rejects an unknown state value", async (ctx) => {
    if (!ready) return ctx.skip(SKIP_REASON);
    const id = await newDraft();
    if (!id) return ctx.skip("seed failed: could not create a draft quote");
