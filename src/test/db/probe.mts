...
for (const a of [rep, est]) {
  const { data, error } = await a.client.rpc("quote_versions_scoped" as any).select("*, changed_by_profile:profiles(id, email, full_name, role)").eq("quote_id", p.id).order("version_number", { ascending: false });
